...
/**
 * 把传入的路径解析为对应的相对于app 预置数据的路径、sdcard路径或其他绝对路径
 */
public class XPathResolver {
    private String mPath;
    private String mPresetDir;
    private Context mContext;

    public enum Scheme {
        NONE, FILE, CONTENT, HTTP, HTTPS, UNKNOWN
    }
     ...
     *            预置数据目录
     */
    public XPathResolver(String path, String presetDir) {
        mPath = path;
     ...
     *            预置数据目录
     * @param context
     *            解析Content协议用到的context
     */
    public XPathResolver(String path, String presetDir, Context context) {
        mPath = path;
        mPresetDir = presetDir;
        mContext = context;
    }
    ...
    public String resolve() {
        ...
        if (null == schemeString) {// 相对预置数据目录的路径
            return resolveRelativePath(mPath);
        ...
        } else if (schemeString.equals(HTTP_SCHEME)
                || schemeString.equals(HTTPS_SCHEME)) {// 解析http协议
            return mPath;
        }
        return null;
    }
     ...
     * @param path
     *            要解析的路径
     * @return 路径在预置数据目录之内，则返回该路径的绝对路径，否则返回null。
     */
    private String resolveRelativePath(String path) {
        File file = new File(mPresetDir, path);
        String absolutePath = null;
        try {
            absolutePath = file.getCanonicalPath();
            // 路径是否在预置数据目录之内
            if (!XFileUtils.isFileAncestorOf(mPresetDir, absolutePath)) {
                absolutePath = null;
            }
        } catch (IOException e) {
            absolutePath = null;
